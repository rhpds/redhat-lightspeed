...
Due to the sensitive nature of the information processed in this environment, no internet connectivity is allowed.  Since Red Hat Lightspeed primarily uses the Hybrid Cloud Console for analysis, historically the use of these types of tools hasn’t been possible since they require a connection to the internet.  +
...
The Red Hat Lightspeed advisor in Satellite feature is now generally available in Satellite 6.19 and offers the advisor capability as well as remediation. +
The Red Hat Lightspeed vulnerability service is generally available in Red Hat Satellite 6.19 and provides vulnerability scanning.

Infinicorp has provided you with a preconfigured Red Hat Satellite 6.19 server and a RHEL 10.1 host to evaluate the capabilities of Red Hat Lightspeed in Satellite.  In the interest of time, all set up tasks for the Satellite server have already been completed for you.
...
1. From your showroom environment, click the tab labeled `rhel-{guid}-2 terminal`.

.rhel-{guid}-2 terminal tab
...
.Output of insights-client command
...
.Red Hat Lightspeed in Satellite Menu
image::Sat_Insights_Connected_Menu.png[../assets/Sat_Insights_Connected_Menu]
...
.OpenSSH Recommendation details page
...
[start=4]
. With a host selected, both the `Remediate` and the `Download playbook` buttons become available. +
These playbooks are created by Red Hat and are supported, but it is always best to review the playbook first to see what it is doing and determine if any modifications are needed. +

. Click `Download playbook`. +
Depending on your browser, additional selections may be needed to download the .yml file.  

. Locate and open the playbook. +
The .yml file should open with any text editor. +
Review the playbook. +
Notice that there are signatures included to help identify that the playbook was created by Red Hat. +
The playbook is generally pretty simple and does the following: +
* Update ClientAliveInterval setting to 300
* Update ClientMaxAlive setting to 0
* Restarts the sshd service so the service accepts the updated values
* Runs the `insights-client` command to identify the issue has been fixed and to remove the recommendation.

This playbook is very simple, and Infinicorp has already approved you running Red Hat provided playbooks in the Satellite test environment. +
This playbook is safe for you to run.

When you have finished reviewing, close the playbook and return to the Satellite UI.
...
[start=9]
. Return to the list of Red Hat Lightspeed recommendations. +
...
=====
.How to see all recommendations
[%collapsible]
====

A common question that gets asked with regards to Red Hat Lightspeed and the recommendations that it offers is how can I see the list of recommendations? + 
For example, are there recommendations for a database like SAP? +

Fortunately this is very easy to see inside of the Satellite user interface (or in the Hybrid Cloud Console). +

From the last section you should be in the Satellite user interface in Red Hat Lightspeed → Recommendations.  

.All Recommendations fixed
image::lb1187_M4_NoRecommendations.png[link=self, window=blank, width=100%]

Notice in the image above that while you have no recommendations, there are still default filters applied. +
This is because Red Hat Lightspeed wants to focus on things that are actively impacting your systems. +
The default filters are `Systems impacted: 1 or more` and `Status: Enabled`.
In other words, show me issues that affect my systems and that are enabled.

To see the full list of recommendations, you will need to remove these filters and then search the list of recommendations.

1. Locate the filters and remove the two default filters by clicking the `x` to the right of them. +
In the image below there are 1951 total recommendations listed. +

.Filters removed
image::lb1187_M4_NoFilters.png[link=self, window=blank, width=100%]

With the full list showing, all you need to do is search.

[start=2]
2. In the `Filter by name` box type *SAP*. +
This will list all Recommendations that have SAP in the name or description. +
In the image below there are 55 recommendations for SAP.

.SAP Recommendations
image::lb1187_M4_SAP.png[link=self, window=blank, width=100%]

Recommendations are commonly created by Red Hat from support cases, best practices guides, or commonly encountered issues.  There are a large variety of Recommendations to help your systems operate to their full potential.
====
=====
